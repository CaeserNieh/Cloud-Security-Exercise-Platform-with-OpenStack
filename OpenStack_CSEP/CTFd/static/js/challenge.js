...
//  var chal_button = $('<button class="chal-button" value="{0}" ><p>{1}</p><span>{2}</span></button>'.format(chal.id, chal.name, chal.value))
        ...
        $('#challenges button').click(function (e) {
            window.location = "/test?name="+this.value;
...
$('.experiment').click(function (e) {
    $('.experiment').text("Already Start Experiment")
    start_experiment()
});
...
function start_experiment(){
  $.get("/experiment_start", function (data) {
    ret_data = [];
    ret_data = $.parseJSON(JSON.stringify(data));
    console.log(ret_data)
    });
}

function get_query(id){
      $.get("/ttt?name="+id, function (data) {
        categories = [];
        challenges = $.parseJSON(JSON.stringify(data));
        console.log(id)
...
$(function() {
    ...
    var para_name = getsearch[1].split("=")[0]
    var para_value = getsearch[1].split("=")[1]
    get_query(para_value)
    // solves_graph()
});
